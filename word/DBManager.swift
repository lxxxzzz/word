...
//        return "\(path)/words.sqlite"
        ...
        var path = "/Users/xiaohongli/Documents/sql/data.sqlite"
    ...
    private override init() {
        ...
        if db.open() {
            ...
            sql = "CREATE TABLE IF NOT EXISTS t_words (book_id TEXT, lesson_id TEXT NOT NULL, number TEXT, english TEXT NOT NULL, soundmark TEXT, chinese TEXT, soundname TEXT, create_time TEXT, update_time TEXT, PRIMARY KEY(english, book_id, lesson_id));"
            if db.executeUpdate(sql, withArgumentsIn: []) {
                print("t_words表创建成功")
            }
...
//                print("\(lesson.title)插入成功\(lesson.words)")
            ...
            for word in lesson.words {
                result = db.executeUpdate("INSERT INTO t_words (english, lesson_id, book_id, number, soundmark,  chinese, soundname, create_time, update_time) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?)", withArgumentsIn: [word.english, lesson.id, book.id, word.number, word.soundmark, word.chinese, word.soundname, Date.sqlDate, Date.sqlDate])
                if result {
